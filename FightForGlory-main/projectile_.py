...
class Projectile_(pygame.sprite.Sprite):
    ...
    def move(self):
        ...
        if self.player.direction == -1:
            ...
            self.rect.x += self.velocity
            self.player.image = pygame.image.load('assets_2/SamuraiAttaqueDroite.png')
            self.player.image = pygame.transform.scale(self.player.image, (140, 150))
            self.image = pygame.image.load('assets_2/Slash.png')
            self.image = pygame.transform.scale(self.image, (70, 130))
        ...
        if self.player.is_full_load_2():
            if self.player.direction == -1:
                self.player.image = pygame.image.load('assets_2/SamuraiGauche.png')
                self.player.image = pygame.transform.scale(self.player.image, (100, 120))
            else:
                self.player.image = pygame.image.load('assets_2/SamuraiDroite.png')
                self.player.image = pygame.transform.scale(self.player.image, (100, 120))
            self.player.all_projectile_.remove(self)
            self.player.cooldown = True